...
# motivo: $raiz vem de $0 — num checkout cujo caminho carregue um
# metacaractere de shell (aspa simples, `$(...)`), emendar o valor na string
# que os corpos abaixo passam a `eval`/`bash -c` faria o caminho virar código
...
# exige_origem_de_forma_valida mede a origem em si, antes de compará-la com
# o artefato — sem ela, uma política "canônica" montada a partir da mesma
# origem que está sendo julgada nunca reprova uma origem perigosa, só uma
# divergência entre build e argumento. Os dois casos que injetam caractere
# perigoso ficam em variável, e o corpo referencia $ORIGEM_TESTE sem
# expandi-lo aqui, pelo mesmo motivo de $ALVO: a aspa e o ponto e vírgula do
# valor não podem virar sintaxe do `eval`/`bash -c` que roda o corpo.
caso "exige_origem_de_forma_valida REPROVA curinga" 1 \
  "source \"\$ALVO\"; exige_origem_de_forma_valida '*'"
caso "exige_origem_de_forma_valida REPROVA esquema nu" 1 \
  "source \"\$ALVO\"; exige_origem_de_forma_valida 'https:'"
caso "exige_origem_de_forma_valida REPROVA caminho depois do host" 1 \
  "source \"\$ALVO\"; exige_origem_de_forma_valida 'http://localhost:3000/caminho'"
caso "exige_origem_de_forma_valida REPROVA esquema que não é http nem https" 1 \
  "source \"\$ALVO\"; exige_origem_de_forma_valida 'ftp://host'"

ORIGEM_TESTE="https://api.folioteca.exemplo; script-src-elem * 'unsafe-inline'"
caso "exige_origem_de_forma_valida REPROVA origem com ponto e vírgula e diretiva colada" 1 \
  "source \"\$ALVO\"; exige_origem_de_forma_valida \"\$ORIGEM_TESTE\""

ORIGEM_TESTE='https://api.exemplo"><script src="https://evil.example/x.js"></script><b c="'
caso "exige_origem_de_forma_valida REPROVA origem com aspas e tag colada" 1 \
  "source \"\$ALVO\"; exige_origem_de_forma_valida \"\$ORIGEM_TESTE\""

caso "exige_origem_de_forma_valida passa com http://localhost:3000" 0 \
  "source \"\$ALVO\"; exige_origem_de_forma_valida 'http://localhost:3000'"
caso "exige_origem_de_forma_valida passa com https://api.folioteca.exemplo" 0 \
  "source \"\$ALVO\"; exige_origem_de_forma_valida 'https://api.folioteca.exemplo'"
