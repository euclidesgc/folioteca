...
| Ações do nó da árvore | `<div data-tree-actions className="flex shrink-0 items-center gap-1">` depois do rótulo, na linha do nó; até quatro ações (pessoas · criar filha · renomear · apagar, nessa ordem) com aparência `ghost` + `size="icon"` — `Button` quando a ação escreve, `<Link>` com `buttonVariants` quando ela navega; sempre visíveis (nunca só no `hover`) | 065 |
