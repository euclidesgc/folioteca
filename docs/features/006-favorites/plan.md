...
- [ ] T1.1 — Contrato primeiro: dois verbos novos, escopo `favorites` e `isFavorite` no documento
...
- [ ] T1.2 — Tabela `Favorite`: modelo, migration à mão e limpeza dos testes
...
- [ ] T1.3 — `FavoritesService`: gravar, apagar e listar, começando sempre pelo acesso
...
- [ ] T1.4 — `isFavorite` no corpo do documento, sem que `DocumentsService` toque a tabela `Favorite`
...
- [ ] T1.5 — Rotas no `DocumentsController`: `PUT`/`DELETE …/favorite` e o despacho da lista por escopo
...
- [ ] T2.1 — API simulada: favoritos no banco em memória, os dois verbos, o escopo novo e a chave de desenvolvimento
...
- [ ] T2.2 — Lista de documentos por escopo e invalidação de todas as listas
...
- [ ] T2.3 — `useUpdateFavorite`: mutação otimista com rollback
...
- [ ] T2.4 — Botão de favorito na página do documento (variante `ghost`, estrela inline, receitas novas)
...
- [ ] T2.5 — Lista e seção da barra lateral parametrizadas por escopo; página "Favoritos" e as duas seções na raiz
...
- [ ] T3.2 — Testes da fase 3 (e2e da jornada de favoritos)
