...
- [x] T2.1 — API simulada: o espaço nasce e morre com a unidade (fecha a dívida 075) e o handler `GET /spaces`
...
- [x] T2.2 — A consulta `['spaces']`, a feature `unit-spaces` no lint e a invalidação pelas mutações de lotação
...
- [x] T2.3 — A seção "Unidades" na barra lateral
...
- [x] T2.4 — A rota `/spaces/:spaceId` e a página do espaço com seus estados
...
- [x] CA2.12 — `git log feature/115-admin-roles-demote..HEAD --format=%s` mostra o commit inicial de documentação da fatia e um commit por fase (1 e 2), todos em inglês e em Conventional Commits.

## Fase 3 — e2e da jornada e documentação: a fatia utilizável de ponta a ponta

Caminhos relativos à raiz do repositório.

- [x] T3.1 — Documentação: a entrega na arquitetura e o roadmap
...
- [x] T3.2 — Testes da fase 3 (e2e: a pessoa lotada chega ao espaço pelo teclado)
