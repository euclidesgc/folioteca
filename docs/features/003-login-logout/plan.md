...
- [ ] T2.1 — Tipo do contrato, caminho `/login` e os dois utilitários de redirecionamento
...
- [ ] T2.2 — API simulada: senha do seed, login e logout
...
- [ ] T2.3 — Sessão no cliente: `useLogin`, `useLogout`, `ProtectedRoute` e o 401 global
...
- [ ] T2.4 — Feature `auth`: formulário de entrar
...
- [ ] T2.5 — Rota `/login`, moldura protegida e fim do aviso "login em breve"
...
- [ ] T2.6 — Botão "Sair" na identidade da barra lateral, com a receita atualizada
...
- [ ] CA2.14 — Nenhum arquivo de `apps/web/src/components`, `lib`, `config`, `types` ou `utils` importa de `@/features` ou `@/app`; não há barrel novo; nenhum token em `localStorage` (`rg -n "localStorage" apps/web/src --glob '!**/testing/**'` vazio) e nenhuma variável `VITE_*` nova.
