...
- [x] T1.1 — Contrato, controller e serviço da remoção
...
- [x] T2.1 — API simulada e chamada da remoção
...
- [x] T2.2 — Controle de nível por linha, confirmação de remoção, foco e anúncios
...
- [x] T3.2 — Testes da fase 3 (e2e: trocar nível, remover com confirmação, acessibilidade)
...
- [x] CA3.4 — `docs/design.md`, na receita "Seletor na linha da lista", tem a variação "com ação destrutiva como última opção" citando "última opção", a confirmação, `aria-disabled`, "nunca `disabled`", `tabIndex={-1}` e o foco após remover; as demais receitas estão como antes.
- [x] CA3.5 — A fatia está utilizável de ponta a ponta: `pnpm exec vitest run --project api` (0) inclui `DELETE share answers 204 and the row is gone` e `after removal the person gets 404 on GET document`; `pnpm exec vitest run --project web` (0) inclui `removing hides the person notifies and focuses the row above` e `changing the level keeps the dialog open and announces the new level`; `pnpm test:e2e` (0) inclui `the owner switches a person from Pode ver to Pode editar in the access list` e `the owner removes a person after confirming and the row disappears`.

## Desvios

Preenchido pelos agentes de fase quando um teste existente precisar de ajuste. Previstos:

- DV1 — `apps/web/src/features/documents/components/__tests__/share-document-dialog.test.tsx`: `sharing with Pode editar shows the Pode editar badge`, `sharing again with Pode ver switches the badge without duplicating the row` e casos da 147 que asserem o selo de nível nas linhas de "Quem tem acesso" (`lists shared people with Pode ver and Pode editar in server order`, `a shared person appears in the list once after sharing`) passam a asserir o valor do `<select>` "Nível de {nome}". Nomes mantidos. O ajudante `selectBeatriz` passa a esperar o grupo "Nível de acesso" em vez do texto "Pode ver", que agora também aparece nas `<option>` das linhas.
...
- DV3 — `apps/api/src/documents/__tests__/documents.contract.test.ts` › `shareDocument path method and personId param match the controller`: a asserção de que o path só tem `put` passou a esperar `['delete', 'put']`, porque a T1.1 acrescentou o `delete` ao mesmo path. Nome mantido (continua verdadeiro).
- DV4 — CA2.2 e CA2.3: literais ajustados à forma implementada, com o mesmo comportamento. Em `db.ts` a função é declarada como `export const removeDocumentShare = (`, então o literal passou de `removeDocumentShare(` para `removeDocumentShare = (`. Em `share-document-dialog.tsx` a linha é um componente único para dono e compartilhamentos, e só a do dono sai da ordem de `Tab`, então o literal passou de `tabIndex={-1}` para `tabIndex={isOwner ? -1 : undefined}`.
- DV5 — T3.2 (`apps/web/e2e/tests/share-level-change.spec.ts`): não existe chave `mock-*` que semeie um documento já compartilhado com duas pessoas (`mock-documents` só tem `sample`, `shared-view`, `shared-edit` e `unit-space`, e criar uma nova mexeria em `apps/web/src/testing/mocks`, fora dos arquivos da tarefa). O estado inicial por `page.addInitScript` continua sendo `mock-installation=signed-in`, `mock-documents=sample` e `mock-people=sample`, e o ajudante do spec compartilha o documento com "Beatriz Nogueira" e "Daniela Prado" pelo próprio diálogo, na mesma página e sem `reload`, antes de cada caso.
